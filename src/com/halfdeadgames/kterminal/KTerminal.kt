...
class KTerminal(width: Int,
    ...
    init {
        ...
        offScreenRenderer = OffScreenRenderer(width = width * scaledCharacterSize,
                height = height * scaledCharacterSize, batch = inputBatch, clearScreen = false)
    ...
    fun update() {
        offScreenRenderer.render(scissor = true) {
            if(needsUpdate) {
                for(x in 0 until width) {
                    for(y in 0 until height) {
                        Gdx.gl.glEnable(GL20.GL_SCISSOR_TEST)
                        Gdx.gl.glScissor(x * scaledCharacterSize, (height - y) * scaledCharacterSize, scaledCharacterSize, scaledCharacterSize)
